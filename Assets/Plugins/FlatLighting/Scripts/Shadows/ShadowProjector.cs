...
namespace FlatLighting {
	...
	public class ShadowProjector : LightSource<ShadowProjector> {
		...
		private const string shadowProjectorCountProperty = "_ShadowProjectors_Length";
		private const string shadowMapMatrixProperty = "_ShadowMapMat";
		private const string shadowModelToViewProperty = "_ShadowMapMV";
		private const string shadowColorProperty = "_ShadowColor";
		private const string shadowBlurProperty = "_ShadowBlur";
		private const string shadowCameraSettingsProperty = "_ShadowCameraSettings";
		private const string shadowTextureProperty = "_ShadowMapTexture";
		private const string DEBUG_SHADOW_TEXTURE_OBJECT_NAME = "DebugShadowMapping";
		...
		private void LateUpdate() {
			#if UNITY_EDITOR
			InitShadowMappingCamera();
			if (!Application.isPlaying || ShadowMapRequested || isRealTime) {
			#else
			if (ShadowMapRequested || isRealTime) {
			#endif
		...
		void SetupDebugMaterial() {
			...
			public static Matrix4x4[] mapMatrix = new Matrix4x4[MAX_LIGHTS];
			public static Matrix4x4[] modelToView = new Matrix4x4[MAX_LIGHTS];
			public static Vector4[] cameraSettings = new Vector4[MAX_LIGHTS];
			public static float[] blur = new float[MAX_LIGHTS];
			public static Vector4[] shadowColor = new Vector4[MAX_LIGHTS];
